<template>
    <div class="flex fixed inset-0 z-20" v-if="show">
        <div v-if="$store.state.showFilterMenu" :class="$store.state.showFilterMenu? 'opacity-50' : 'opacity-0'" class="flex transition-all delay-500 fixed inset-0 bg-black" @click="$store.state.showFilterMenu=false"></div>
        <transition name="slideleft">
            <div :class="$store.state.showFilterMenu ? 'move-in' : 'move-out'" class="transition-all delay-500 max-w-full relative flex flex-col bg-gray-100 w-80">
                <div class="w-full flex justify-end bg-white">
                    <button @click="$store.state.showFilterMenu=false" aria-label="Search button" class="item p-1 bg-gray-100 rounded-md m-1 hover:bg-gray-200">
    ...
    export default {
        ...
        watch: {
            "$route.params": {
                handler() {
                    this.$store.state.showFilterMenu = false
                },
                deep: true
            },
            "$store.state.showFilterMenu"(val){
                if(val){
                    this.show = val;
                }else{
                    setTimeout(() => {
