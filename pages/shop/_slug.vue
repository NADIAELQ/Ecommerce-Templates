    ...
    <div class="container">
      ...
      <div class="w-1/2 p-8">
        <nav class="text-gray-600 mb-4">
          <a href="/" class="hover:text-gray-800">Home</a> &gt; <span> {{ $settings.sections.shop.header.title }} </span>
        </nav>
        <h1 class="text-4xl font-bold mb-2">{{ $settings.sections.shop.header.title }}</h1>
        <p class="text-lg text-gray-600">{{ $settings.sections.shop.header.description }}</p>
      </div>
      <div class="w-1/2">
        <si-image class="w-full h-full object-cover "
          :src="$settings.sections.shop.header.image ? $settings.sections.shop.header.image.src : $store.state.defaults.shop_image"
          :alt="$settings.sections.shop.header.title"
            :property="'width'"
        />
      </div>
    </div>
    <div class="relative flex mb-2 justify-between items-center px-4 py-12 border-b">
      <!-- <button @click="toggleSideBar = true" aria-label="Filter button" class="flex items-center px-4 py-2 border border-black rounded-full hover:bg-primary"> -->
    <button @click="$store.state.showFilterMenu = !$store.state.showFilterMenu" aria-label="Filter button" class="flex items-center px-4 py-2 border border-black rounded-full hover:bg-primary">
        <span class="mr-2">FILTRES</span>
        <svg role="presentation" fill="none" focusable="false" stroke-width="2" width="20" height="14" class="icon-subdued ml-6 icon icon-filter" viewBox="0 0 20 14">
      ...
      <select class="flex items-center px-4 py-2 border border-black rounded-full hover:bg-primary" v-model="params.sort">
        <option class="p-2" v-for="(sort, i) in sorts" :key="i" :value="sort.field">{{ sort.name }}</option>
      </select>
    </div>
        <div class="relative flex mb-2">
            <transition name="slideleft">
                <div v-if="$store.state.showFilterMenu" class="fixed inset-0 z-40 flex bg-black bg-opacity-50">
                    <!-- <div class="fixed inset-0 block bg-black bg-opacity-50 md:hidden" @click="$store.state.showFilterMenu=false"></div> -->
                    <div class="w-3/4 md:w-2/6 bg-white overflow-auto p-4 mt-4 mb-4 ml-4">
                        <div class="flex justify-end mb-4">
                            <button @click="$store.state.showFilterMenu=false" aria-label="Close button" class="p-1 bg-gray-100 rounded-md hover:bg-gray-200">
                                <svg aria-hidden="true" focusable="false" data-prefix="fal" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" class="w-5 h-5 ">
                                <path fill="currentColor" d="M193.94 256L296.5 153.44l21.15-21.15c3.12-3.12 3.12-8.19 0-11.31l-22.63-22.63c-3.12-3.12-8.19-3.12-11.31 0L160 222.06 36.29 98.34c-3.12-3.12-8.19-3.12-11.31 0L2.34 120.97c-3.12 3.12-3.12 8.19 0 11.31L126.06 256 2.34 379.71c-3.12 3.12-3.12 8.19 0 11.31l22.63 22.63c3.12 3.12 8.19 3.12 11.31 0L160 289.94 262.56 392.5l21.15 21.15c3.12 3.12 8.19 3.12 11.31 0l22.63-22.63c3.12-3.12 3.12-8.19 0-11.31L193.94 256z" class=""></path>
                                </svg>
                            </button>
                        </div>
                        <h2 v-if="$settings.sections.shop.sidebar.collections.active" class="px-2 p-2">{{ $settings.sections.shop.sidebar.collections.title }}</h2>
                        <div v-if="$settings.sections.shop.sidebar.collections.active" class="flex flex-col mb-2">
                            ...
                            <div v-for="(item, i) in collections" :key="i" class="px-2">
                                <div class="flex items-center">
                                    ...
                                    <label v-if="item.childrens && item.childrens.length == 0 " class="capitalize p-2 cursor-pointer collec-name" :for="item.slug">{{ item.name }}</label>
                        ...
                        <h2 v-if="$settings.sections.shop.sidebar.prices.active" class="px-2 mt-2 p-2">{{ $settings.sections.shop.sidebar.prices.title }}</h2>
                        ...
                        <h2 class="px-2 p-2" v-if="$settings.sections.shop.sidebar.sizes.active">{{ $settings.sections.shop.sidebar.sizes.title }}</h2>
                        ...
                        <h2 class="px-2 p-2" v-if="$settings.sections.shop.sidebar.colors.active">{{ $settings.sections.shop.sidebar.colors.title }}</h2>
                        ...
                        <div v-if="$settings.sections.shop.sidebar.tags.active && filters" class="flex flex-col mb-2">
                            <div v-for="(tag, i) in filters.tags" :key="i" class="flex items-center px-2 p-2">
                        ...
                        <h2 class="px-2 p-2" v-if="$settings.sections.shop.sidebar.brands.active">{{ $settings.sections.shop.sidebar.brands.title }}</h2>
                        <div class="flex flex-col mb-2 ">
                            <div v-if="$settings.sections.shop.sidebar.brands.active && loading.brands" class="flex items-center justify-center my-5 p-2">
                                <si-loader></si-loader>
                            </div>
                            <div v-if="$settings.sections.shop.sidebar.brands.active">
                                <div v-for="(item, i) in brands" :key="i" class="flex items-center px-2 p-2">
            ...
            <div class="w-full ">
                <div class="bg-white">
                    ...
                    <div v-if="items.length>0" class="flex items-center mt-6 justify-center w-full p-2 border-t mb-12">
                    <button aria-label="" class="flex items-center px-6 py-1 border border-black rounded-full bg-green-700">
                        <button class="flex items-center p-2 text-white " @click="getItems(paginate.current_page-1)">
                        ...
                        <span>&ensp;</span>
                        <span class="text-white">{{paginate.current_page}}/{{paginate.last_page}}</span>
                        <span>&ensp;</span>
                        <button class="flex items-center p-2 text-white " @click="getItems(paginate.current_page+1)">
